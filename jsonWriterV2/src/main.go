...
func main() {
	...
	// get Directories
	text, err := os.Open("/home/mz/goTools/jsonsBytxt3/searchDirectory.txt")
	if err != nil {
		log.Fatal(err)
	}

	// set result directories
        resultDirectories, err := ioutil.ReadFile("/home/mz/goTools/jsonsBytxt3/dirForResults.txt")
        if err != nil {
            log.Fatal(err)
        }
        resultDirectory := string(resultDirectories[:])
	...
	for scanner.Scan() {
		...
		var dirName = filepath.Dir(file)
		fmt.Println("directory : " + dirName)
		var baseName = filepath.Base(file)
		...
		dbPool.Debug().Raw("select worker_name, worker_sex, worker_age, worker_home, worker_loc, worker_tool, worker_dialect FROM worker WHERE worker_id = ?", items[0]).Scan(&userInfo)

                if len(userInfo.Worker_name) == 0 {
                    userInfo.Worker_name = "ZZZ"
                }
                if len(userInfo.Worker_sex) == 0 {
                    userInfo.Worker_sex = "Z"
                }
                if len(userInfo.Worker_age) == 0 {
                    userInfo.Worker_age = "Z"
                }
                if len(userInfo.Worker_home) == 0 {
                    userInfo.Worker_home = "Z"
                }
		...
		var worker_name = userInfo.Worker_name
                fmt.Println("name : " + worker_name)
                if worker_name == "ZZZ" {
                    worker_name = "NotProvided"
                }
                var worker_gender = userInfo.Worker_sex
                fmt.Println("gender : " + worker_gender)
                var worker_age = userInfo.Worker_age
                fmt.Println("age : " + worker_age)
                var worker_region = userInfo.Worker_home
                ...
                // for edited versions
                //dbNS.Debug().Raw("select response from rp WHERE wavpath like ? and submit_datetime is not null", "%"+fileName+"%").Scan(&command)
                dbNS.Debug().Raw("select response from rp WHERE wavpath = ? and submit_datetime is not null", file).Scan(&command)

		// get duration
                dbNS.Debug().Raw("select wav_dur_sec from rp WHERE wavpath = ? and submit_datetime is not null", file).Scan(&duration)
                ...
                */
                ...
                if len(command) == 0 {
                    ...
                    data[0].File_Info.FileLength = duration
                    data[0].File_Info.FileFormat = "PCM"
                    data[0].File_Info.NumberOfRepeat = "1"
                    data[0].File_Info.TimeInterval = "0"
                    ...
                    // save json file
                    err := ioutil.WriteFile(resultDirectory+"/"+fileName+".json", doc, os.FileMode(0644))
                    if err != nil {
                        fmt.Println(err)
                    }

		    // var command initialization
		    command = ""
                   ...
                   // if deleted, move all
                   if isDeleted {
                       oldPathJson := resultDirectory+"/"+fileName+".json"

                       text2, err := ioutil.ReadFile("/home/mz/goTools/jsonsBytxt3/dirForDeleted.txt")
                       if err != nil {
                        log.Fatal(err)
                       }
                       newPath := string(text2[:])
                       newPathJson := newPath+"/"+fileName+".json"

                       // move json
                       jsonErr := os.Rename(oldPathJson,newPathJson)
                       if jsonErr != nil {
                           log.Fatal(jsonErr)
                       }
                   }
