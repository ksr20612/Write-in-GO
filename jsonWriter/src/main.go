...
func main() {
	...
	// choose Directory
	text, err := ioutil.ReadFile("/home/mz/jsonMaker/jsonForFinished/searchDirectory.txt")
	if err != nil {
		log.Fatal(err)
	}
	...
	e = filepath.Walk(Directory, func(path string, info os.FileInfo, err error) error {
		...
		if err == nil && libRegEx.MatchString(info.Name()) {
            ...
            // check if json already exists (processes all done)
            //var jsonPath = dirName + fileName + ".json"
            //fmt.Println("jsonName : " + jsonPath)
            //if _, jsonExists := os.Stat(jsonPath)

            // if json does not exist(= (not edited) || deleted )
            //if os.IsNotExist(jsonExists) {

                // open WAV file & get duration
                f, readErr := os.Open(path)
                if readErr != nil {
                    log.Fatal(readErr)
                }
                length, err2 := wav.NewDecoder(f).Duration()
                //dur := strconv.FormatFloat(length.Seconds(),'E','-1',32)[0:3]
                dur := fmt.Sprintf("%.2f", length.Seconds())
                if err2 != nil {
                    log.Fatal(err2)
                }
                f.Close()
                ...
                dbNS.Debug().Raw("select response from rp WHERE wavpath = ? and submit_datetime is not null", path).Scan(&command)
                ...
                */
                ...
                if len(command) == 0 {
                    ...
                    data[0].File_Info.FileLength = dur
		    ...
		    command = ""
                   ...
                   // if deleted, move all
                   if isDeleted {
                       oldPathJson := dirName+"/"+fileName+".json"

                       text2, err := ioutil.ReadFile("/home/mz/jsonMaker/jsonForFinished/dirForDeleted.txt")
                       if err != nil {
                        log.Fatal(err)
                       }
